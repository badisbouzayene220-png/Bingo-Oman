(function(){
  ...
  function driverBlock(driver,assignment){if(!assignment)return '<div class="bct-driver"><strong>المندوب</strong><div class="bct-driver-meta"><span>لم يتم تعيين مندوب بعد</span></div></div>';const name=driver?.display_name||'تم تعيين مندوب';const online=driver?.is_online===true?'🟢 متصل':driver?.is_online===false?'⚫ غير متصل':'';const phone=driver?.phone?`<span>📞 ${esc(driver.phone)}</span>`:'';const loc=(driver?.latitude!=null&&driver?.longitude!=null)?`<a class="bct-map-link" target="_blank" rel="noopener" href="https://www.google.com/maps?q=${encodeURIComponent(driver.latitude+','+driver.longitude)}">📍 مشاهدة موقع المندوب</a>`:'';return `<div class="bct-driver"><strong>🛵 ${esc(name)}</strong><div class="bct-driver-meta"><span>${online}</span>${phone}<span>الحالة: ${esc(assignment.status||'assigned')}</span></div>${loc}</div>`;}
  ...
  async function loadTracking(silent=true){
    ...
    if(!silent){refreshBtn?.classList.add('busy');refreshBtn&&(refreshBtn.textContent='جاري التحديث...');}
    try{
      ...
      const ids=orders.map(o=>o.id);let assignments=[];
      try{const a=await sb.from('delivery_assignments').select('id,order_id,driver_id,status,offered_at').in('order_id',ids).order('offered_at',{ascending:false});if(!a.error)assignments=a.data||[];}catch(e){console.warn('BINGO customer assignments:',e);}
      const latestAssignment=new Map();assignments.forEach(a=>{if(!latestAssignment.has(a.order_id))latestAssignment.set(a.order_id,a);});
      const driverIds=[...new Set(assignments.map(a=>a.driver_id).filter(Boolean))];let drivers=[];
      if(driverIds.length){try{const d=await sb.from('delivery_drivers').select('id,display_name,phone,is_online,latitude,longitude,last_online_at').in('id',driverIds);if(!d.error)drivers=d.data||[];}catch(e){console.warn('BINGO customer drivers:',e);}}
      const driverMap=new Map(drivers.map(d=>[d.id,d]));

      const html=orders.map(o=>{const a=latestAssignment.get(o.id);const d=a?driverMap.get(a.driver_id):null;const statusClass=o.status==='delivered'?'done':o.status==='cancelled'?'cancel':'';return `<article class="bct-order"><div class="bct-top"><div class="bct-no">#${esc(o.order_number||o.id)}</div><span class="bct-status ${statusClass}">${esc(statusLabels[o.status]||o.status)}</span></div>${timeline(o.status)}<div class="bct-grid"><div class="bct-info"><small>الإجمالي</small><strong>${Number(o.total||0).toFixed(3)} OMR</strong></div><div class="bct-info"><small>عنوان التوصيل</small><strong>${esc(o.delivery_address||'—')}</strong></div></div>${driverBlock(d,a)}</article>`;}).join('');
      const snapshot=orders.map(o=>[o.id,o.status,o.total,o.delivery_address||'']).flat().join('|');
      if(host.dataset.snapshot!==snapshot){host.innerHTML=html;host.dataset.snapshot=snapshot;window.dispatchEvent(new CustomEvent('bingo:tracking-rendered'));}
    ...
    finally{if(!silent){refreshBtn?.classList.remove('busy');refreshBtn&&(refreshBtn.textContent='تحديث');}}
  }
