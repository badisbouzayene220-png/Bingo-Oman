(function(){
  'use strict';
  if(window.__bingoCustomerTrackingV3Loaded)return;
  window.__bingoCustomerTrackingV3Loaded=true;
  ...
  async function loadTracking(silent=true){
    ...
    try{
      ...
      if(!rows.length){host.innerHTML='<div class="bct-empty">لا توجد طلبات للمتابعة حاليًا.</div>';return;}
      const html=rows.map(orderHtml).join('');
      const snapshot=JSON.stringify(rows.map(r=>[r.order_id,r.order_status,r.driver_id,r.driver_name,r.assignment_status,r.driver_online,r.driver_location_updated_at,r.delivery_address]));
      ...
      console.error('BINGO customer tracking RPC:',e);
      host.innerHTML='<div class="bct-error">تعذر تحميل متابعة الطلب: '+esc(e?.message||String(e))+'</div>';
    }finally{
      if(!silent){refreshBtn?.classList.remove('busy');if(refreshBtn)refreshBtn.textContent='تحديث';}
    }
  }
