...
as $$ declare v_user uuid:=auth.uid(); v_pkg public.bingo_package_catalog%rowtype; v_order uuid;
begin
 ...
 if exists(select 1 from public.bingo_purchase_orders o where o.user_id=v_user and o.package_code=p_package_code and o.status='pending' and o.listing_id is not distinct from p_listing_id) then
   select id into v_order from public.bingo_purchase_orders o where o.user_id=v_user and o.package_code=p_package_code and o.status='pending' and o.listing_id is not distinct from p_listing_id order by created_at desc limit 1; return v_order;
 end if;
 insert into public.bingo_purchase_orders(user_id,package_code,package_kind,listing_id,amount_baisa,customer_note) values(v_user,v_pkg.code,v_pkg.kind,p_listing_id,v_pkg.amount_baisa,nullif(trim(p_customer_note),'')) returning id into v_order; return v_order;
end;$$;
...
as $$ select o.id,o.package_code,o.package_kind,o.listing_id,l.title::text,o.amount_baisa,o.currency,o.status,o.customer_note,o.admin_note,o.created_at,o.decided_at from public.bingo_purchase_orders o left join public.listings l on l.id=o.listing_id where o.user_id=auth.uid() order by o.created_at desc; $$;

-- profiles has no email column in this project; email is stored in auth.users.
...
begin
 ...
 return query
 select o.id,o.user_id,coalesce(p.username,'')::text,coalesce(u.email,'')::text,o.package_code,o.package_kind,o.listing_id,l.title::text,o.amount_baisa,o.currency,o.status,o.customer_note,o.admin_note,o.created_at,o.decided_at
 from public.bingo_purchase_orders o
...
as $$ declare v_role text; v_order public.bingo_purchase_orders%rowtype; v_pkg public.bingo_package_catalog%rowtype; v_end timestamptz;
begin
 select role::text into v_role from public.profiles where id=auth.uid(); if coalesce(v_role,'')<>'admin' then raise exception 'Admin access required'; end if;
 ...
 if v_pkg.kind='promotion' then
   update public.listings set promotion_type=v_pkg.code,promoted_at=now(),promotion_expires_at=v_end where id=v_order.listing_id and user_id=v_order.user_id and status='published'; if not found then raise exception 'Published listing no longer available'; end if;
 else
 ...
 update public.bingo_purchase_orders set status='approved',admin_note=p_admin_note,decided_at=now(),approved_by=auth.uid() where id=p_order_id; return true;
end;$$;
