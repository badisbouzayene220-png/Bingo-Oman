-- BINGO fractional checkout fix V2
-- Forces product stock/order item quantity to NUMERIC(18,3)
-- and replaces place_store_order() so kg/meter/liter quantities like 1.5 work.

begin;

-- Save RLS policies that may depend on affected columns.
create temporary table _bingo_saved_policies_v2 on commit drop as
...
declare r record;
begin
  for r in select * from _bingo_saved_policies_v2 loop
    execute format('drop policy if exists %I on %I.%I',r.policyname,r.schemaname,r.tablename);
  end loop;
end $$;

-- Convert only if needed; safe to rerun when already numeric.
...
-- Restore original RLS policies exactly.
do $$
declare
  r record;
  v_roles text;
...
begin
  for r in select * from _bingo_saved_policies_v2 loop
    ...
    v_sql := format(
      ...
      r.policyname,r.schemaname,r.tablename,r.permissive,r.cmd,coalesce(v_roles,'public')
    );
...
-- Replace the current checkout RPC. Quantity is NUMERIC, never INTEGER.
...
declare
  ...
  qty numeric(18,3);
  sub numeric(18,3) := 0;
  fee numeric(18,3) := greatest(coalesce(p_delivery_fee,0),0);
...
begin
  ...
  insert into public.store_orders(
    id,order_number,user_id,customer_name,phone,email,address,notes,payment_method,subtotal,delivery_fee,total
  ) values(
    order_id,order_no,uid,trim(p_customer_name),trim(p_phone),nullif(trim(coalesce(p_email,'')),''),
    trim(p_address),nullif(trim(coalesce(p_notes,'')),''),p_payment_method,0,fee,0
  );

  for item in select * from jsonb_array_elements(p_items) loop
    begin
      qty := round((item->>'quantity')::numeric,3);
    ...
    -- Only piece/pack must be whole numbers.
    ...
    insert into public.store_order_items(
      ...
      order_id,p.id,p.title,p.price,qty,round((p.price*qty)::numeric,3)
    );
  ...
  update public.store_orders
    set subtotal=sub,total=sub+fee,updated_at=now()
    where id=order_id
    returning * into r;
