...
-- Fixes remaining INTEGER casts in checkout/delivery bridge functions.
-- Safe to run after V2.

begin;

-- Ensure delivery order item quantities are decimal-capable too.
alter table public.delivery_order_items
  alter column quantity type numeric(18,3) using quantity::numeric;

-- Patch known checkout/delivery functions in-place, preserving their current logic.
-- We only replace casts applied specifically to JSON quantity values.
do $$
declare
  r record;
  def text;
...
-- Re-assert the base delivery creator with NUMERIC quantity handling.
