...
with check (bucket_id='seller-products' and (storage.foldername(name))[1]=auth.uid()::text);
...
using (bucket_id='seller-products' and owner_id=auth.uid())
with check (bucket_id='seller-products' and owner_id=auth.uid());
