(function () {
  ...
  // Original text is stored once per DOM node. Translation runs only on
  // initial load and explicit language/user actions.
  const PAIRS = {
  ...
  function remember(root=document) {
    root.querySelectorAll('*').forEach(el => {
      if (el.children.length === 0 && el.textContent.trim() && !originals.has(el)) originals.set(el, el.textContent);
      ['placeholder','title','aria-label'].forEach(a => {
        const key='erpHrOrig'+a.charAt(0).toUpperCase()+a.slice(1);
        if (el.hasAttribute(a) && !el.dataset[key]) el.dataset[key]=el.getAttribute(a);
      });
    });
  }
  ...
  document.addEventListener('click',function(e){
    ...
    // User actions can create ERP/HR modals or tables. Capture their new nodes,
    // but never run automatically in the background.
