...
begin
 ...
 return coalesce((select jsonb_agg(to_jsonb(x) order by x.purchase_date desc,x.created_at desc) from (
   select p.*,s.name supplier_name,s.company_name supplier_company,
    ...
    coalesce((select sum(jl.debit-jl.credit) from public.erp_journal_entries je join public.erp_journal_lines jl on jl.entry_id=je.id join public.erp_accounts a on a.id=jl.account_id where je.source_type='purchase_cancellation' and je.source_id=p.id and a.code='1210'),0) refund_receivable,
    coalesce((select sum(sr.amount) from public.erp_supplier_refunds sr where sr.purchase_id=p.id),0) refunded_amount,
    greatest(
      coalesce((select sum(jl.debit-jl.credit) from public.erp_journal_entries je join public.erp_journal_lines jl on jl.entry_id=je.id join public.erp_accounts a on a.id=jl.account_id where je.source_type='purchase_cancellation' and je.source_id=p.id and a.code='1210'),0)
      - coalesce((select sum(sr.amount) from public.erp_supplier_refunds sr where sr.purchase_id=p.id),0),0) refund_balance
