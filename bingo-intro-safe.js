...
'use strict';
const DURATION=7200;
(function ensureV5Css(){const old=document.querySelector('link[href*="bingo-intro-safe.css"]');if(old)old.href='bingo-intro-safe.css?v=5';else{const l=document.createElement('link');l.rel='stylesheet';l.href='bingo-intro-safe.css?v=5';document.head.appendChild(l)}})();
...
function init(){
  ...
  const ar=isArabic();const root=document.createElement('div');root.id='bingoSafeIntro';root.setAttribute('role','dialog');root.setAttribute('aria-modal','true');root.setAttribute('aria-label','BINGO Oman introduction');root.innerHTML=`
    ...
    <div class="bsi-stage">
      ...
      <div class="bsi-phase bsi-logo-phase"><div class="bsi-logo-fallback" aria-hidden="true">BINGO <span>OMAN</span></div><img class="bsi-logo" src="assets/bingo-oman-intro.png?v=5" alt="BINGO Oman"></div>
      <div class="bsi-phase bsi-mascot-phase"><div class="bsi-mascot-card"><img class="bsi-mascot" src="assets/characters/bingo-mascot-phone-v2.png?v=5" alt="BINGO Oman mascot and Oman map"></div></div>
  ...
  if(logo){logo.addEventListener('load',()=>root.classList.add('bsi-logo-loaded'),{once:true});logo.addEventListener('error',()=>{logo.src='logo-en.png?v=5';logo.addEventListener('error',()=>logo.style.display='none',{once:true})},{once:true})}
  if(mascot){mascot.addEventListener('load',()=>root.classList.add('bsi-mascot-loaded'),{once:true});mascot.addEventListener('error',()=>{mascot.src='assets/characters/main.png?v=5';mascot.addEventListener('error',()=>mascot.style.display='none',{once:true})},{once:true})}
  let closed=false;const close=()=>{if(closed)return;closed=true;try{sessionStorage.setItem('bingoCinematicIntroSeenV4','1')}catch(e){}root.classList.add('is-closing');window.setTimeout(()=>root.remove(),620)};root.querySelector('.bsi-skip')?.addEventListener('click',close,{once:true});document.addEventListener('keydown',e=>{if(e.key==='Escape'||e.key==='Enter')close()},{once:true});const timer=window.setTimeout(close,DURATION);window.addEventListener('pagehide',()=>clearTimeout(timer),{once:true});
}
