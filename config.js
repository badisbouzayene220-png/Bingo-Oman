...
(function(){
  ...
  if(/(?:^|\/)dashboard\.html$/i.test(location.pathname)){
    js('dashboard-listing-analytics.js?v=20260822-2','dashboard-listing-analytics-loader');
  }
  ...
  function addSummary(){
    ...
    if(!box){box=document.createElement('div');box.id='delivery-earnings-summary';box.style.cssText='display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin:14px 0';const section=earnings.closest('.bd-card')||earnings.parentElement;section.parentNode.insertBefore(box,section)}
    const totals={pending:0,approved:0,paid:0,total:0};
    [...earnings.querySelectorAll('tr')].forEach(tr=>{const cells=[...tr.children];if(cells.length<4)return;const status=(cells[3].textContent||'').trim().toLowerCase();const raw=(cells[2].textContent||'').replace(/[^0-9.\-]/g,'');const amount=Number(raw||0);if(!Number.isFinite(amount))return;if(Object.prototype.hasOwnProperty.call(totals,status))totals[status]+=amount;totals.total+=amount});
    const signature=[totals.pending,totals.approved,totals.paid,totals.total].join('|');if(box.dataset.signature===signature)return;box.dataset.signature=signature;
    const card=(title,value,cls)=>`<div class="bd-card" style="margin:0"><div style="opacity:.7">${title}</div><div style="font-size:24px;font-weight:800" class="${cls||''}">${value.toFixed(3)} OMR</div></div>`;box.innerHTML=card('Pending',totals.pending,'warning')+card('Approved',totals.approved,'warning')+card('Paid',totals.paid,'success')+card('الإجمالي',totals.total,'')}
  document.addEventListener('DOMContentLoaded',()=>{if(!location.pathname.endsWith('bingo-delivery-admin.html'))return;addSummary();new MutationObserver(addSummary).observe(document.body,{childList:true,subtree:true})});
})();